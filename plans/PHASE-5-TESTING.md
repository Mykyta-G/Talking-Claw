...
python trigger.py "Test call — checking if everything works"
```
...
| LLM too slow (cloud) | Use Claude Haiku instead of Sonnet | -500ms |
...
| Issue | Cause | Fix |
|-------|-------|-----|
| Robotic / choppy voice | Sample rate mismatch | Resample 48kHz↔16kHz |
...
| Wake word detection | "Hey Clawd" triggers call | +0.5 GB |
